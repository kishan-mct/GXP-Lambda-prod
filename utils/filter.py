...
def filter_condition_and_params(filters):
    condition = "1=1"
    params = []
...
def filter_execute_query(table_name, columns, filters, page_size, page_number,order_by):
    condition, params = filter_condition_and_params(filters)
    return gxp_db.select_query(
        table_name,
        columns=columns,
        condition=condition,
        params=params,
