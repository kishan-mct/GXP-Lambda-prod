...
def roomUpdateDestroy(event, context):
    ...
    try:
        ...
        if http_method == 'PATCH':
            ...
            query_result = gxp_db.update_query("room_room", request_body, condition="id=%s", params=(room_type_id,))
            if query_result['status']:
                query_result = gxp_db.get_query("room_room", "*", condition="id=%s", params=(room_type_id,))

        elif http_method == 'DELETE':
                query_result = gxp_db.delete_query("room_room", condition="id=%s", params=(room_type_id,))  
