...
def authGroupListCreateUpdateDestroy(event, context):
    ...
    try:
        if http_method == 'GET':
            ...
            if 'ag.id' in multi_value_qp:
                ag_id = multi_value_qp.pop('ag.id')[0]
                table = "auth_group ag " \
                ...
                query_result = gxp_db.get_query(table, columns, condition="ag.id=%s GROUP BY ag.id", params=(ag_id,))
                
            else:
                query_result = gxp_db.select_query("auth_group", order_by=order_by, page_size=page_size, page_number=page_number)

        elif http_method == 'POST':
            request_body = convert_empty_strings_to_none(json.loads(event['body']))
            name = request_body.get('name')
            hotel_id = request_body.get('hotel_id')
