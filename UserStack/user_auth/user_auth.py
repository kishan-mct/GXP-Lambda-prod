...
def get_user_access_token_by_login(user_id, remote_ip, is_last_login_update=False, hotel_id=None):
    ...
    user_detail_query = f"""
        SELECT
            ...
            COALESCE(
                jsonb_agg(
                    jsonb_build_object('id', ap.id, 'name', ap.name, 'is_active', ap.is_active)
                ) FILTER (WHERE ap.id IS NOT NULL),
                '[]'::jsonb
            ) AS access_app_platforms
        FROM
            users_user u
        LEFT JOIN
            app_platform ap ON ap.id = ANY(
                SELECT jsonb_array_elements_text(u.access_app_platform)::uuid
            )
        WHERE
            u.id = '{user_id}' AND u.is_active = TRUE
        GROUP BY
            u.id;
    """
    get_user = gxp_db.raw_query_fetchone(user_detail_query).get("data", {})
    ...
    try:
        if http_method == 'GET':
            ...
            if not user_data['is_gxp_staff']:
                ...
                if len(users_userhotel_id_list) == 1:
                    condition += f" AND id = '{users_userhotel_id_list[0]}'"
                else:
                    condition += f" AND id IN {tuple(users_userhotel_id_list)}"
